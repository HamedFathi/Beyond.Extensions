...
internal static class SingularRules
{
    internal static Dictionary<Regex, string> GetRules()
    {
        return new Dictionary<Regex, string>
        {
            ...
            {
                new Regex("((a)naly|(b)a|(d)iagno|(p)arenthe|(p)rogno|(s)ynop|(t)he)(?:sis|ses)$",
                    RegexOptions.IgnoreCase),
                "$1sis"
            },
            { new Regex("(^analy)(?:sis|ses)$", RegexOptions.IgnoreCase), "$1sis" },
            {
                new Regex("(wi|kni|(?:after|half|high|low|mid|non|night|[^\\w]|^)li)ves$", RegexOptions.IgnoreCase),
                "$1fe"
            },
            { new Regex("(ar|(?:wo|[ae])l|[eo][ao])ves$", RegexOptions.IgnoreCase), "$1f" },
            { new Regex("ies$", RegexOptions.IgnoreCase), "y" },
            {
                new Regex("\\b([pl]|zomb|(?:neck|cross)?t|coll|faer|food|gen|goon|group|lass|talk|goal|cut)ies$",
                    RegexOptions.IgnoreCase),
                "$1ie"
            },
            ...
            {
                new Regex("(x|ch|ss|sh|zz|tto|go|cho|alias|[^aou]us|tlas|gas|(?:her|at|gr)o|ris)(?:es)?$",
                    RegexOptions.IgnoreCase),
                "$1"
            },
            ...
            {
                new Regex("(apheli|hyperbat|periheli|asyndet|noumen|phenomen|criteri|organ|prolegomen|hedr|automat)a$",
                    RegexOptions.IgnoreCase),
                "$1on"
            },
